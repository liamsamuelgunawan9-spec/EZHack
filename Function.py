...
def perform_ip_geolocation(target: str) -> str:
    ...
    if clean_host.startswith("+") or (clean_host.isdigit() and len(clean_host) > 6):
        try:
            ...
            rand_lon = round(random.uniform(106.6, 107.0), 4) if "Indonesia" in region_location or "+62" in parsed_phone else round(random.uniform(106.6, 107.0), 4)
...
# Custom Javascript definitions and block generators for the UI Injection
BLOCKLY_INJECTS = """
